...
module.exports = function(group, element, elementTemplates, bpmnFactory, translate) {
  ...
  var renderOutputParameter = function(id, p) {
  ...
  var entries = [];
  var id, outputEntries;

  // filter specific output parameters from template
  var outputParameters = filter(template.properties, function(p) {
    return p.binding.type === CAMUNDA_OUTPUT_PARAMETER_TYPE && p.type === INPUT_OUTPUT_DEFAULT_TYPE;
  });

  forEach(outputParameters, function(property, idx) {

    id = 'template-outputs-' + template.id + '-' + idx;

    outputEntries = renderOutputParameter(id, property);
    if (outputEntries) {
      entries = entries.concat(outputEntries);
    }
  });
...
function setPropertyValue(element, property, value, bpmnFactory) {
  ...
  if (bindingType === CAMUNDA_OUTPUT_PARAMETER_TYPE) {
    ...
    var existingOutputParameter = findOutputParameter(inputOutput, binding);

    var newOutputParameter = createOutputParameter(binding, value, bpmnFactory);
    ...
    updates.push(cmdHelper.addAndRemoveElementsFromList(
      ...
      null,
      [ newOutputParameter ],
      existingOutputParameter ? [ existingOutputParameter ] : []
    ));
  }
