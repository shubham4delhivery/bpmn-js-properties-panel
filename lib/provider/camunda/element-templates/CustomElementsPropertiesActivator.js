...
var ELEMENT_TEMPLATE_INPUT_PARAMETER = /^template-inputs-/;

var ELEMENT_TEMPLATE_OUTPUT_PARAMETER = /^template-outputs-/;
...
function isTemplateInputEntry(entry) {
  return ELEMENT_TEMPLATE_INPUT_PARAMETER.test(entry.id);
}

function isTemplateOutputEntry(entry) {
  return ELEMENT_TEMPLATE_OUTPUT_PARAMETER.test(entry.id);
}
...
function isEntryVisible(entry, template) {
  ...
  if (
    entryId === 'elementTemplate-chooser' ||
    isCustomEntry(entry) ||
    isTemplateInputEntry(entry) ||
    isTemplateOutputEntry(entry)
  ) {
    return true;
  }
