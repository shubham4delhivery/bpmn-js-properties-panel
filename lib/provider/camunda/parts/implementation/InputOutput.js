...
var InputOutputParameter = require('./InputOutputParameter2');
...
module.exports = function(element, bpmnFactory, options, translate) {
  ...
  if (ensureOutparameterSupported(element, insideConnector)) {
    entries.push(
      getParametersHeading(element, bpmnFactory, {
        idPrefix: idPrefix,
        label: translate('Output Variables'),
        insideConnector: insideConnector,
...
function getParametersHeading(element, bpmnFactory, options) {
  ...
  var entry = {
    ...
    html: '<div class="bpp-field-wrapper">' +
      '<label class="bpp-input-output__header">' + utils.escapeHTML(options.label) + '</label>' +
      '<button type="button" class="bpp-input-output__add add" ' + 'data-action="createElement">' +
        '<span>+</span>' +
...
function getInputParameterEntries(element, bpmnFactory, options, translate) {
  ...
  var inputParameters = params.map(function(param, index) {
    ...
    return InputOutputParameter(param, bpmnFactory, { idPrefix: idPrefix + '_inputParameter_' + index, onRemove: onRemove, onToggle: onToggle }, translate);
  });
...
function getOutputParameterEntries(element, bpmnFactory, options, translate) {
  ...
  var outputParameters = params.map(function(param, index) {
    ...
    return InputOutputParameter(param, bpmnFactory, { idPrefix: idPrefix + '_outputParameter_' + index, onRemove: onRemove, onToggle: onToggle }, translate);
  });
