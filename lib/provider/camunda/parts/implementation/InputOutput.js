...
module.exports = function(element, bpmnFactory, options, translate) {
  ...
  var params = getInputParameters(element, insideConnector);
  var inputs = params.map(function(param, index) {
    function onRemove(_, entry) {
      params.splice(index, 1);
    }

    return InputOutputParameter(param, bpmnFactory, { idPrefix: 'custom' + index, onRemove: onRemove }, translate);
  });

  entries.push.apply(entries, inputs.map(function(input) { return input.entries; }).flat());
