...
InputOutputHelper.validateVariableExpression = function(value) {
  ...
  if (isLiteral(expressionBody)) {
    return { value: 'Value mus not contain literals.' };
  }
 ...
 * @return {Object}
 */
InputOutputHelper.getParameterType = function(bo, forceType) {
  ...
  if (forceType) {
    return parameterTypes[forceType];
  }
  ...
  // (4) validate whether value is <constant-value>
  if (!this.validateConstantValue(value)) {
    return parameterTypes['constant-value'];
  }

  // (5) validate whether value is <variable>
  if (!this.validateVariableExpression(value)) {
    return parameterTypes['variable'];
  }
