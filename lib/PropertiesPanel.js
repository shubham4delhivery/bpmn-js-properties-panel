		...
		if (element) {
			if (e.elements.indexOf(element) !== -1) {
				if (element.businessObject.$type === "bpmn:StartEvent") {
					objectChanged =
						current.entries.Object.oldValues.Object !== element.businessObject.Object;
					if (objectChanged) {
	...
	if (values instanceof Array) {
		...
		for (var i = 0; i < values.length; i++) {
			...
			if (entry.validateListItem) {
				var validationErrors = entry.validateListItem(
						current.element,
						listValue,
						entryNode,
						i
		...
		forEach(groups, function (group) {
			...
			forEach(group.entries, function (entry) {
				...
				var entryNode = domify(
					'<div class="bpp-properties-entry" data-entry="' +
						escapeHTML(entry.id) +
						'"></div>'
				);
